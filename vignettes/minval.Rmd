...
Another step for a stoichiometric reactions validation is to check their mass - charge balance. This process requires the use of a reference with chemical formulas, molecular weights or net charges for each metabolite included in the metabolic model.
...
In a balanced stoichiometric reaction according to the Lomonosov-Lavoisier law, the mass comprising the reactants should be the same mass present in the products. If chemical formula is given, the `checkBalance` function multiplies the atom numbers by their respective stoichiometric coefficient, and establishes if the atomic composition of reactants and products are the same. If molecular weight is given then sum of masses for reactants and products are compared. In both cases a boolean `TRUE` value is returned if mass is balanced.
...
```
When all stoichiometric reactions are balanced, then the model can be exported and loaded to be interrogated again:
```{r}
writeSBML(modelData = glycolysis,
