...
```
### SBML files
The standard format to share and store biological processes such as metabolic models is the Systems Biology Markup Language (SBML) format. **MINVAL** package includes the `"writeSBML"` function which is able to write models in SBML (`level = 2`, `version = 1`) format as follows:
```{r}
...
```
### TSV files
The function `writeTSV` writes a metabolic model in three text files, following a character-separated value format. Each line contains one entry; the default value separator is a tab. TSV models are the default input format for the `sybil` R package.
```{r}
